...
func newClient(t *testing.T) fanart.Client {
	const (
		replayApiKey string = "fake-api-key"
	)
	replayMode := getReplayMode(t)
	clientOptions := fanart.ClientOptions{}
	switch replayMode {
	case replayAppend, replayReplace, replayDirect:
		clientOptions.APIKey = os.Getenv("FANART_API_KEY")
		if clientOptions.APIKey == "" {
			t.Fatal("FANART_API_KEY must be set for replace, append, or direct modes when using API key")
		}
	case replayRead:
	...
	return fanart.NewClient(clientOptions)
}
